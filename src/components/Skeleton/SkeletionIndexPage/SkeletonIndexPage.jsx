...
const SkeletonIndexPage = () => {
  return (
    <div className="h-screen mt-4 px-10 xl:px-20">
      <div className="cursor-pointer flex-shrink-0 flex gap-10 items-center justify-center py-3 animate-pulse overflow-hidden">
        {Array.from({ length: 30 }).map((_, index) => (
          <div className="max-h-16 flex flex-col justify-center items-center gap-2" key={index}>
      ...
      <div className="my-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6  gap-6">
        {Array.from({ length: 6 }).map((_, index) => (
          <div class="relative animate-pulse">
            <div class="h-[340px] sm:h-[260px] md:h-[350px] lg:h-[258px] bg-gray-200 rounded-xl"></div>
            <div class="mt-3">
      ...
      <div className="mt-20 space-y-5">
        <h3 class="text-6xl font-medium animate-pulse">
          <div class="bg-gray-200 rounded w-[800px] h-[40px]"></div>
        </h3>
      </div>
      <div className="my-5 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6  gap-6">
        {Array.from({ length: 6 }).map((_, index) => (
